...
public class StringUtil {
	 ...
	 * <p>
	 * <p>
	 * This is useful for quickly splitting a string directly into an array of tokens, instead of an enumeration of tokens (as <code>StringTokenizer</code> does).
	 * 
	 * @param str
	 *            The string to parse.
	 * @param separator
	...
	/**
	 * Replace a string with another string inside a larger string, for the first <code>max</code> values of the search string. A <code>null</code> reference is passed to this method is a no-op.
	 * 
	 * @param text
	 *            text to search and replace in
	 * @param repl
	...
	/**
	 * 用Map中的变量-变量值替换源字符串中的变量名,这个方法返回字符串是从char[] 构成的UTF字符,不需要指定任何字符集都不可能乱码 以前那个方法是把UTF字符串又转换成GBK的byte,以要重新指定字符集为GBK是为了和其它的GBK字符同时输出,以要把UTF字符串转换成GBK的字符串以便同时显示.
	 */
	...
	public static String reFormatDateString(String pattern, String dateString) {
		Date date = StringUtil.parseDateString(pattern, dateString);
	...
	public static String formatSubString(Object str, String str1) {
		...
		if (str != null) {
			...
			if (tempIndex == -1) {
				...
				tempStr = tempString.substring(0, tempIndex);
			}
		} else {
			tempStr = "";
		}
	...
	public static boolean isStringNotBlank(String str) {
		if (str == null || (str.trim().length() == 0) || (str.equalsIgnoreCase("null")) || (str.equals(""))) {
			return false;
		} else {
			return true;
		}
