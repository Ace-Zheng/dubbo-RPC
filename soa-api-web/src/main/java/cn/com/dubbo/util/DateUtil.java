...
//		rightNow.add(Calendar.DAY_OF_YEAR,-1);//日期加10天
	...
	/**
	 * 计算两个时间间隔的小时数，保留两位小数(时间差)
	 * @param startTime
	 * @param endTime
	 * @return
	 */
	public static String diffHours(String startTime,String endTime){
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		DecimalFormat dft = new DecimalFormat("0.00");//double小数点
		try {
			if(StringUtil.isBlank(startTime)||StringUtil.isBlank(endTime)){
				return "0";
			}
			
		    long diff = sdf.parse(startTime).getTime() - sdf.parse(endTime).getTime();
		    double minis = diff / (1000);//计算秒数
		    double hours = minis/(60*60)*100/100;
		    return  dft.format(hours);
		} catch (Exception e) {
	...
	public static void main(String[] args) {
		...
		System.out.println(DateUtil.diffHours("2016-11-24 00:06:00","2016-11-24 00:05:00"));
	}
}
